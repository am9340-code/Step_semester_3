\# Step\_semester\_3



step program weekly assignments (java)



\## Date: 08-08-2026



\*\*Today's Work:\*\*

\- Completed Week 3 Control Flow class problems.

\- Organized the Control Flow class problems under the Week 3 package.

\- Completed problems on Armstrong numbers, GCD, Palindrome, Prime Number, and Star Triangle.

\- Added the Week 3 work to the `feature/session\_3` branch.



\*\*Next Session Plan:\*\*

\- Continue with the next session's programming problems.

\- Maintain the required GitHub branching and package structure.



\*\*Issues Faced:\*\*

\- Faced issues while moving the Control Flow class problems into the Week 3 package and organizing the Git branches.

\- Corrected the package and branch structure according to the required workflow.



\---



\## Date: 01-08-2026



\*\*Today's Work:\*\*

\- Completed Week 2 class problems.

\- Completed Week 2 assignment problems.

\- Organized the problems into the Week 2 class and assignment packages.

\- Added the Week 2 work to the `feature/session\_2` branch.



\*\*Next Session Plan:\*\*

\- Continue with Week 3 Control Flow problems.

\- Maintain the required feature branch structure.



\*\*Issues Faced:\*\*

\- Faced some Git branching and package organization issues while setting up the session.

\- Corrected the branch and package organization before continuing.



\---



\## Date: 29-08-2026



\*\*Today's Work:\*\*

\- Completed Week 1 class problems.

\- Completed Week 1 assignment problems.

\- Organized the problems into the Week 1 class and assignment packages.

\- Added the Week 1 work to the `feature/session\_1` branch.



\*\*Next Session Plan:\*\*

\- Continue with Week 2 problems.

\- Maintain the required feature branch structure.



\*\*Issues Faced:\*\*

\- Initially faced confusion regarding the required GitHub branching structure.

\- Corrected the structure so that the coding work remains on the appropriate feature branches.

